"""MCP protocol-specific assertion primitives."""
...
def assert_result_exists(response: dict[str, Any]) -> list[str]:
    """Assert that a result exists (successful response)."""
    body = response.get("body", {})
    if "result" not in body:
        return ["Response has no 'result' field"]
    if "error" in body:
        return ["Response has both 'result' and 'error' fields"]
    return []


def assert_id_matches_request(response: dict[str, Any], request_id: Any) -> list[str]:
    """Assert response ID matches the request ID."""
    body = response.get("body", {})
    actual_id = body.get("id")
    if actual_id != request_id:
        return [f"Response id '{actual_id}' does not match request id '{request_id}'"]
    return []
